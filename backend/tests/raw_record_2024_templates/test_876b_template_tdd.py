...
class Template876BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-876B 直流通用分压器测量系统.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-876B 直流通用分压器测量系统.docx', 'file_size': 19375, 'table_count': 2, 'text_length': 561, 'has_general_check_keyword': True, 'general_check_length': 48, 'uncertainty_items_count': 0, 'measured_items_count': 0}
