...
class Template809BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-809B 氧弹、空气弹老化试验箱.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-809B 氧弹、空气弹老化试验箱.docx', 'file_size': 20060, 'table_count': 2, 'text_length': 662, 'has_general_check_keyword': True, 'general_check_length': 35, 'uncertainty_items_count': 0, 'measured_items_count': 0}
