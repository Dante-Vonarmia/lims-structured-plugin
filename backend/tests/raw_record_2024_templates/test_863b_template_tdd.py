...
class Template863BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-863B 冲击电压测量系统.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-863B 冲击电压测量系统.docx', 'file_size': 21961, 'table_count': 4, 'text_length': 812, 'has_general_check_keyword': True, 'general_check_length': 86, 'uncertainty_items_count': 0, 'measured_items_count': 0}
