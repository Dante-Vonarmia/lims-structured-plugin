...
class Template854BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-854B 炭黑含量试验装置.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-854B 炭黑含量试验装置.docx', 'file_size': 19608, 'table_count': 2, 'text_length': 784, 'has_general_check_keyword': True, 'general_check_length': 29, 'uncertainty_items_count': 0, 'measured_items_count': 0}
