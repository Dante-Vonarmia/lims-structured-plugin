...
class Template847BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-847B 静摩擦系数试验仪.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-847B 静摩擦系数试验仪.docx', 'file_size': 20960, 'table_count': 1, 'text_length': 758, 'has_general_check_keyword': True, 'general_check_length': 22, 'uncertainty_items_count': 0, 'measured_items_count': 0}
