...
class Template869BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-869B 煤矿用电缆负载燃烧试验机.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-869B 煤矿用电缆负载燃烧试验机.docx', 'file_size': 19752, 'table_count': 2, 'text_length': 716, 'has_general_check_keyword': True, 'general_check_length': 68, 'uncertainty_items_count': 0, 'measured_items_count': 0}
