...
class Template824BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-824B 剥离试验仪.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-824B 剥离试验仪.docx', 'file_size': 19645, 'table_count': 2, 'text_length': 560, 'has_general_check_keyword': True, 'general_check_length': 32, 'uncertainty_items_count': 0, 'measured_items_count': 0}
