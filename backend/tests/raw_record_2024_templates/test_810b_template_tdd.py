...
class Template810BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-810B 工频、直流火花试验机.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-810B 工频、直流火花试验机.docx', 'file_size': 19745, 'table_count': 2, 'text_length': 750, 'has_general_check_keyword': True, 'general_check_length': 46, 'uncertainty_items_count': 0, 'measured_items_count': 0}
