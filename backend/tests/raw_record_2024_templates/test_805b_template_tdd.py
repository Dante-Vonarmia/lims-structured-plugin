...
class Template805BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-805B 耐磨试验装置.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-805B 耐磨试验装置.docx', 'file_size': 19378, 'table_count': 2, 'text_length': 620, 'has_general_check_keyword': True, 'general_check_length': 47, 'uncertainty_items_count': 0, 'measured_items_count': 0}
