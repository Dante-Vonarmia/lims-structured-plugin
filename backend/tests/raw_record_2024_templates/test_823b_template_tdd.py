...
class Template823BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-823B 单根绝缘电线电缆垂直燃烧试验装置.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-823B 单根绝缘电线电缆垂直燃烧试验装置.docx', 'file_size': 19022, 'table_count': 2, 'text_length': 764, 'has_general_check_keyword': True, 'general_check_length': 40, 'uncertainty_items_count': 0, 'measured_items_count': 0}
