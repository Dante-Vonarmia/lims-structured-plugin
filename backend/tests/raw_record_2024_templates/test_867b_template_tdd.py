...
class Template867BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-867B 直流电阻试验装置（低电阻表）.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-867B 直流电阻试验装置（低电阻表）.docx', 'file_size': 19233, 'table_count': 2, 'text_length': 518, 'has_general_check_keyword': True, 'general_check_length': 35, 'uncertainty_items_count': 0, 'measured_items_count': 0}
