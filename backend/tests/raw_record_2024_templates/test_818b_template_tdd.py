...
class Template818BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-818B 高压漆膜连续性试验仪.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-818B 高压漆膜连续性试验仪.docx', 'file_size': 23388, 'table_count': 4, 'text_length': 1304, 'has_general_check_keyword': True, 'general_check_length': 60, 'uncertainty_items_count': 0, 'measured_items_count': 0}
