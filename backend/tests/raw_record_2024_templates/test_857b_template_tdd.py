...
class Template857BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-857B 单根铜芯绝缘细电线电缆垂直燃烧试验装置.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-857B 单根铜芯绝缘细电线电缆垂直燃烧试验装置.docx', 'file_size': 18637, 'table_count': 2, 'text_length': 590, 'has_general_check_keyword': True, 'general_check_length': 50, 'uncertainty_items_count': 0, 'measured_items_count': 0}
