...
class Template878BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-878B 工频大电流测量系统.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-878B 工频大电流测量系统.docx', 'file_size': 19026, 'table_count': 2, 'text_length': 431, 'has_general_check_keyword': True, 'general_check_length': 50, 'uncertainty_items_count': 0, 'measured_items_count': 0}
