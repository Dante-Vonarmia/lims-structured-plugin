...
class Template859BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-859B 工频高电压测量系统.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-859B 工频高电压测量系统.docx', 'file_size': 19608, 'table_count': 2, 'text_length': 652, 'has_general_check_keyword': True, 'general_check_length': 39, 'uncertainty_items_count': 0, 'measured_items_count': 0}
