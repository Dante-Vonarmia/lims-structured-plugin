...
class Template826BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-826B 恒温水浴.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-826B 恒温水浴.docx', 'file_size': 20211, 'table_count': 2, 'text_length': 676, 'has_general_check_keyword': True, 'general_check_length': 18, 'uncertainty_items_count': 0, 'measured_items_count': 0}
