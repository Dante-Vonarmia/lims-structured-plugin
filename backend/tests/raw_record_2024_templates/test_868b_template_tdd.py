...
class Template868BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-868B 直流电阻试验装置（电桥）.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-868B 直流电阻试验装置（电桥）.docx', 'file_size': 19410, 'table_count': 2, 'text_length': 557, 'has_general_check_keyword': True, 'general_check_length': 35, 'uncertainty_items_count': 0, 'measured_items_count': 0}
