...
class Template835BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-835B 火花机人工击穿装置.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-835B 火花机人工击穿装置.docx', 'file_size': 22657, 'table_count': 3, 'text_length': 1079, 'has_general_check_keyword': True, 'general_check_length': 50, 'uncertainty_items_count': 0, 'measured_items_count': 0}
