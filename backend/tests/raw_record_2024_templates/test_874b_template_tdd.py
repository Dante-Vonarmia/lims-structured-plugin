...
class Template874BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-874B 绕组线卷绕试验仪.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-874B 绕组线卷绕试验仪.docx', 'file_size': 18311, 'table_count': 2, 'text_length': 403, 'has_general_check_keyword': True, 'general_check_length': 24, 'uncertainty_items_count': 0, 'measured_items_count': 0}
