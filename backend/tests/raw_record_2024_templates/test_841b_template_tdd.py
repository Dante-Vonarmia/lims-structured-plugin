...
class Template841BTDD(unittest.TestCase):
    def test_template_snapshot_regression(self) -> None:
        path = RAW_RECORD_2024_DIR / 'R-841B 高绝缘电阻测量仪（高阻计）.docx'
        if not path.exists():
            self.skipTest(f"template not found: {path}")
        got = collect_docx_snapshot(path)
        expected = {'file_name': 'R-841B 高绝缘电阻测量仪（高阻计）.docx', 'file_size': 19170, 'table_count': 2, 'text_length': 351, 'has_general_check_keyword': True, 'general_check_length': 20, 'uncertainty_items_count': 0, 'measured_items_count': 0}
