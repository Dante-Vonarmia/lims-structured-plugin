...
import { createEmptyFields, createInitialState } from "../core/state/factory.js";
    ...
    function renderRichCellHtml(text) {
    ...
    function hasDocxImageToken(text) {
      return /\[\[DOCX_IMG\|/.test(String(text || ""));
    }

    function collectDocxImageTokens(text, limit = 2) {
    ...
    function enrichGeneralCheckWithDocxImages(blockText, rawText) {
    ...
    function normalizeValidationToken(value) {
    ...
    async function generateItem(item, generateMode = "certificate_template") {
      ...
      const payload = {
        ...
        fields: {
          ...
          raw_record: item.rawText || item.fields.raw_record || "",
        },
      };
    ...
    function normalizeOptionalBlank(value) {
      const text = String(value || "").trim();
      if (!text) return "";
      if (/^[\/\\;；|]+$/.test(text)) return "";
    ...
    function extractBlockByLine(raw, startPatterns = [], endPatterns = []) {
    ...
    function extractAllBlocksByLine(raw, startPatterns = [], endPatterns = []) {
      const lines = splitRawLines(raw);
      if (!lines.length) return "";
      const blocks = [];
      let idx = 0;
    ...
    function parseDateFromLabelText(text, labelPattern) {
      const source = String(text || "");
    ...
    function isCompleteDateText(value) {
    ...
    function shiftDateText(value, deltaDays) {
    ...
    function inferDateTriplet(input = {}) {
    ...
    function parseTableRowsFromBlock(blockText) {
      ...
      const parseInstrumentBlockTable = () => {
        ...
        // Catalog-based parsing is only a fallback and must not drive initial recognition.
        ...
        // OCR often breaks a single cell into multiple lines; rebuild as 7-column rows.
    ...
    function shouldRebuildMeasurementItemsFromRaw(currentText, item) {
      ...
      for (const row of body) {
        ...
        // Typical misalignment in screenshot: model contains date, code contains name,
        // range contains uncertainty text, uncertainty/cert columns contain cert/date.
    ...
    function buildGeneralCheckWysiwygData(blockText) {
    ...
    function renderGeneralCheckStructuredTable(tableStruct) {
      ...
      const cutIdx = tableRowsRaw.findIndex((row) => {
        const line = (row.texts || []).join(" ");
        return /(?:^|\s)注[:：]?(?:\s|$)/.test(line) || /(?:以下空白|\(以下空白\)|（以下空白）)/.test(line);
      });
      const tableRows = (cutIdx >= 0 ? tableRowsRaw.slice(0, cutIdx) : tableRowsRaw).map((x) => x.html);
      return `
    ...
    function renderGeneralCheckWysiwygBlock(blockText, options = {}) {
      ...
      const tableStruct = (options && options.tableStruct && typeof options.tableStruct === "object")
        ? options.tableStruct
        : null;
      const data = buildGeneralCheckWysiwygData(blockText);
      ...
      if (readOnly && tableStruct) {
        const structuredHtml = renderGeneralCheckStructuredTable(tableStruct);
        if (structuredHtml) {
          return `
      ...
      const bodyHtml = rows.map((row, rowIdx) => `<tr>${row.map((cell, colIdx) => {
        if (readOnly) {
          if (data.twoColumn && colIdx === 0) {
            return `<td style="width:120px;">${renderRichCellHtml(cell)}</td>`;
          }
          return `<td>${renderRichCellHtml(cell)}</td>`;
        }
        if (data.twoColumn && colIdx === 0) {
    ...
    function renderStructuredBlockHtml(blockText, options = {}) {
      ...
      if (generalCheckRows) {
        const bodyHtml = generalCheckRows
          .map((row) => `<tr><td>${renderRichCellHtml(row[0] || "")}</td><td>${renderRichCellHtml(row[1] || "")}</td></tr>`)
          .join("");
    ...
    function bindEvents() {
      ...
      const handleTargetFieldChange = (event) => {
        ...
        if (key === "general_check_cell") {
          ...
          const multiColumnTable = parseGeneralCheckMultiColumnTable(String(item.fields.general_check_full || ""));
          if (multiColumnTable) {
            const [header, ...body] = multiColumnTable;
            const working = (body.length ? body : [Array(header.length).fill("")]).map((row) => row.map((cell) => String(cell || "")));
            ...
            const rows = parseGeneralCheckRowsForEditor(String(item.fields.general_check_full || ""));
            const working = (rows.length ? rows : [["", ""]]).map((row) => [String((row && row[0]) || ""), String((row && row[1]) || "")]);
            if (rowIdx < 0 || rowIdx >= working.length) return;
            if (colIdx < 0 || colIdx > 1) return;
            working[rowIdx][colIdx] = String(nextCellValue || "");
            item.fields.general_check_full = buildGeneralCheckTextFromRows(working);
          }
